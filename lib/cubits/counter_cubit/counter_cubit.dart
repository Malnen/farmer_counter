...
class CounterCubit extends Cubit<CounterState> {
  ...
  CounterCubit({Isar? isar, DriveSyncService? syncService})
    ...
    _initFuture = _init();
    this.syncService.isarNotifier.addListener(_onIsarChanged);
  }

  @override
  Future<void> close() async {
    syncService.isarNotifier.removeListener(_onIsarChanged);
    return super.close();
  }

  Future<CounterItem?> addItem(String name) => _chainOperation(() async {
        name = name.trim();
        if (name.isEmpty) {
          return null;
        }
  ...
  Future<void> removeItem(String guid, {VoidCallback? afterDelete}) => _chainOperation(() async {
  ...
  Future<void> updateItem(CounterItem updatedItem) => _chainOperation(() async {
  ...
  Future<void> applyDelta({required String guid, required int delta}) => _chainOperation(() async {
        final CounterItem? item = await _isar.counterItems.filter().guidEqualTo(guid).findFirst();
        if (item == null) {
          return;
        }
  ...
  Future<void> deleteHistoryEntry(String guid, int changeId) => _chainOperation(() async {
  ...
  Future<CounterChangeItem?> getLatestChange(String guid) =>
      _chainOperation(() => _isar.counterChangeItems.where().counterGuidEqualToAnyAt(guid).sortByAtDesc().findFirst());
  ...
  void _onIsarChanged() {
    if (!isClosed) {
      _isar = syncService.isarNotifier.value;
      _loadItems();
    }
  }

  Future<void> _loadItems() => _chainOperation(() async {
  ...
  Future<T> _chainOperation<T>(Future<T> Function() action) async {
    if (!_initCompleter.isCompleted) {
      await _initFuture;
    }
